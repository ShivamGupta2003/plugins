...
export default function Environments() {
  ...
  const tableData: EnvironmentTableData[] = (environments || []).map(env => {
    ...
    const recipes = env.properties.recipes || {};
    const recipesCount = Object.values(recipes).reduce(
      (sum, resourceTypeRecipes) =>
        resourceTypeRecipes && typeof resourceTypeRecipes === 'object'
          ? sum + Object.keys(resourceTypeRecipes).length
          : sum,
      0
    );
    const provisioningState =
      'provisioningState' in env.properties
